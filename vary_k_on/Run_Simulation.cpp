...
void Run_Simulation(std::string filename, int torque_flag, int k_on_id, double k_on_val, int brute_force_flag, int world_rank, int restartflag)
{
	double force = 1.0;
	...
	std::string outputfolder = "outputfiles/RUN_" + std::to_string(torque_flag) + "_" + std::to_string(k_on_id) + "_" + std::to_string(brute_force_flag);
	std::string inputfolder = "inputfiles/RUN_" + std::to_string(torque_flag) + "_" + std::to_string(k_on_id) + "_" + std::to_string(brute_force_flag);
...
int main(int argc, char *argv[])
{
	...
	int brute_force_flag = std::stoi(argv[6]);

	generator = std::mt19937(std::time(NULL) + torque_flag + k_on_id + brute_force_flag + world_rank);

	try
	{
		Run_Simulation(filename, torque_flag, k_on_id, k_on_val, brute_force_flag, world_rank, restartflag);
	}
