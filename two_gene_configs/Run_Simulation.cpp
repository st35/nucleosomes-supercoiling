...
void Run_Simulation(std::string filename, int config_id, int k_on_id, double k_on_val, int brute_force_flag, int world_rank, int restartflag)
{
	double force = 1.0;
	...
	promoters[1] = k_on_val;

	finish_count_limit = 50;

	std::string outputfolder = "outputfiles/RUN_" + std::to_string(config_id) + "_" + std::to_string(k_on_id) + "_" + std::to_string(brute_force_flag);
	std::string inputfolder = "inputfiles/RUN_" + std::to_string(config_id) + "_" + std::to_string(k_on_id) + "_" + std::to_string(brute_force_flag);
...
int main(int argc, char *argv[])
{
	...
	generator = std::mt19937(std::time(NULL) + config_id + k_on_id + brute_force_flag + world_rank);

	try
	{
		Run_Simulation(filename, config_id, k_on_id, k_on_val, brute_force_flag, world_rank, restartflag);
	}
