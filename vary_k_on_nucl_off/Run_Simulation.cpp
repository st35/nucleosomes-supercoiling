...
void Run_Simulation(std::string filename, int torque_flag, int k_on_id, double k_on_val, int nucl_off_id, double nucl_off_val, int brute_force_flag, int world_rank, int restartflag)
{
	double force = 1.0;
	...
	if(k_on_val < 1e-2)
	{
		finish_count_limit = 5;
	}
	else if(k_on_val < 1e-1)
	{
		finish_count_limit = 20;
	}
	else
	{
		finish_count_limit = 100;
	}

	std::string outputfolder = "outputfiles/RUN_" + std::to_string(k_on_id) + "_" + std::to_string(nucl_off_id) + "_" + std::to_string(brute_force_flag);
	std::string inputfolder = "inputfiles/RUN_" + std::to_string(k_on_id) + "_" + std::to_string(nucl_off_id) + "_" + std::to_string(brute_force_flag);

	Gillespie_Simulation_Multiple_Genes(force, restartflag, brute_force_flag, torque_flag, gene_lengths, TSSes, Gene_Directions, clamp0_flag, clamp1_flag, T, finish_count_limit, promoters, topoisomerase, rna_degrad, barrier_on, barrier_off, barrier, nucl_par, fileflag, outputfolder, inputfolder, world_rank, single_pol_II_flag, nucl_file_flag);
...
int main(int argc, char *argv[])
{
	...
	int torque_flag = std::stoi(argv[3]);
	int k_on_id = std::stoi(argv[4]);
	double k_on_val = std::stod(argv[5]);
	int nucl_off_id = std::stoi(argv[6]);
	double nucl_off_val = std::stod(argv[7]);
	int brute_force_flag = std::stoi(argv[8]);

	generator = std::mt19937(std::time(NULL) + torque_flag + k_on_id + nucl_off_id + brute_force_flag + world_rank);

	try
	{
		Run_Simulation(filename, torque_flag, k_on_id, k_on_val, nucl_off_id, nucl_off_val, brute_force_flag, world_rank, restartflag);
	}
