...
void Run_Simulation(std::string filename, int torque_flag, int subsequent, int brute_force_flag, int world_rank, int restartflag)
{
	double force = 1.0;
	...
	int single_pol_II_flag = subsequent;

	int nucl_file_flag = 1;

	std::string outputfolder = "outputfiles/RUN_" + std::to_string(torque_flag) + "_" + std::to_string(subsequent) + "_" + std::to_string(brute_force_flag);
	std::string inputfolder = "inputfiles/RUN_" + std::to_string(torque_flag) + "_" + std::to_string(subsequent) + "_" + std::to_string(brute_force_flag);
...
int main(int argc, char *argv[])
{
	...
	int subsequent = std::stoi(argv[4]);
	int brute_force_flag = std::stoi(argv[5]);

	generator = std::mt19937(std::time(NULL) + torque_flag + subsequent + brute_force_flag + world_rank);

	try
	{
		Run_Simulation(filename, torque_flag, subsequent, brute_force_flag, world_rank, restartflag);
	}
