...
std::vector<double> Get_Supercoiling_Densities_With_Barriers(int clamp0_flag, int clamp1_flag, const std::vector<double> &phi_x, const std::vector<double> &Segments, const std::vector<int> &Alive, const std::vector<double> &Barr, const std::vector<int> &Barr_Status, const std::vector<double> &Barr_Phi) // Get the supercoiling densities in the segments defined by RNAPs and protein barriers
{
	...
	for(int i = 0; i < allphi.size(); i++)
	{
		if(i == 0)
		{
			...
			{
				right_sigma = (allphi[i] - 0.0) / (w0*Segments[segmentindex]);
			}
	...
	else
	{
		...
		{
			left_sigma = (0.0 - allphi[allphi.size() - 1]) / (w0*Segments[Segments.size() - 1]);
		}
...
double Get_Equalizing_Phi(const std::vector<double> &Segments, const std::vector<double> &Barr, double eqpos, const std::vector<double> &phi_x, const std::vector<int> &Alive, const std::vector<int> &Barr_Status, const std::vector<double> &Barr_Phi) // Get the value of phi in the middle of a segment such that if an RNAP / barrier protein were placed at that point, the two segments on either side will have the same supercoiling density
{
