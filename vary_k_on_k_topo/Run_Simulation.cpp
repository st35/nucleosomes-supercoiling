...
void Run_Simulation(std::string filename, int torque_flag, int k_on_id, double k_on_val, int topo_id, double topo_val, int brute_force_flag, int world_rank, int restartflag)
{
	double force = 1.0;
	...
	std::string outputfolder = "outputfiles/RUN_" + std::to_string(torque_flag) + "_" + std::to_string(k_on_id) + "_" + std::to_string(topo_id);
	std::string inputfolder = "inputfiles/RUN_" + std::to_string(torque_flag) + "_" + std::to_string(k_on_id) + "_" + std::to_string(topo_id);
...
int main(int argc, char *argv[])
{
	...
	generator = std::mt19937(std::time(NULL) + torque_flag + k_on_id + topo_id + brute_force_flag + world_rank);

	try
	{
		Run_Simulation(filename, torque_flag, k_on_id, k_on_val, topo_id, topo_val, brute_force_flag, world_rank, restartflag);
	}
